...
layui.config({
    ...
    $('#huName').text(user.userAccount);
    laytpl(sideNav.innerHTML).render(user.role.menuList, function (html) {
        $('.layui-layout-admin .layui-side .layui-nav').html(html);
        element.render('nav');
    });
    index.regRouter(user.role.menuList);  // 注册路由
    index.loadHome({  // 加载主页
